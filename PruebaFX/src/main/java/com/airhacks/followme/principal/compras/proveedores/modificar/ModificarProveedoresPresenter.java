 ...
 */
package com.airhacks.followme.principal.compras.proveedores.modificar;
 ...
 * @author diego
 */
public class ModificarProveedoresPresenter implements Initializable {
    ...
    public void ingresar() {
        if(!camposVacios()){
            ...
            if(verificarRut(rutS)){
                ...
                String[] rut_new = proveedor.getRut().split("-");
                provModel.modificarProveedor(data, Integer.parseInt(rut_new[0]));
    ...
    public void cargarDatos(){
        ...
        reg.getSelectionModel().select(data.get(7));
        int region = reg.getSelectionModel().getSelectedIndex();
        cargarCiudades(region);
        cargarComunas(region);
        ciu.getSelectionModel().select(data.get(8));
        com.getSelectionModel().select(data.get(9));
